-- Query for dependent objects referencing columns in tables and views with SMALLINT datatype
SELECT DISTINCT
    OBJECT_NAME(dep.referencing_id) AS ReferencingObject,
    SCHEMA_NAME(o.schema_id) AS SchemaName,
    OBJECT_NAME(c.object_id) AS ObjectName,
    SCHEMA_NAME(o.schema_id) + '.' + OBJECT_NAME(c.object_id) AS FullObjectName,
...
-- Query for dependent objects referencing parameters in stored procedures and functions
SELECT DISTINCT
    OBJECT_NAME(dep.referencing_id) AS ReferencingObject,
    SCHEMA_NAME(o.schema_id) AS SchemaName,
...
-- Query for dependent objects referencing locally declared variables in stored procedures and functions
SELECT DISTINCT
    OBJECT_NAME(dep.referencing_id) AS ReferencingObject,
    SCHEMA_NAME(o.schema_id) AS SchemaName,
...
WHERE 
	...
	and dep.referencing_id is not null
ORDER BY 
    SchemaName, ObjectName, ReferencingObject;
